...
def get_rnd_hand():
    """Sélection de 5 cartes aléatoires différentes pour un nouveau jeu."""
    ...
    # Vérification s’il y a une carte en double si oui lancement de :get_rnd_hand().
    if Pok.card_one == Pok.card_two or Pok.card_one == Pok.card_three or  \
       ...
       Pok.card_three == Pok.card_five or  Pok.card_four == Pok.card_five:
        #relancer si carte en double
        get_rnd_hand()

def get_rnd_card_if_not_held():
    """Si la carte n'est pas garder, et que on pioche, fournir des nouvelles cartes."""
    ...
    #Vérification s’il y a une carte en double parmis celle non gardé
    if Pok.card_one == Pok.card_two or Pok.card_one == Pok.card_three or  \
        Pok.card_two == Pok.card_three or Pok.card_one == Pok.card_four or  \
        Pok.card_two == Pok.card_four or Pok.card_three == Pok.card_four or  \
        Pok.card_one == Pok.card_five or Pok.card_two == Pok.card_five or   \
...
# Verification des combinaison de carte gagnante dans la main.
def check_royal_flush(hand):
    """Verification de la quinte flush."""
    if check_flush(hand) and check_straight(hand):
        # Check de la quinteflush.
...
def check_full_house(hand):
    """3 cartes similaire et une paire."""
    values = [i[0] for i in hand]
    value_counts = defaultdict(lambda: 0)
    for v in values:
        value_counts[v] += 1
...
def check_one_pairs(hand):
    ...
    # Réinitialiser le contrôle des paires gagnantes avant.
    pair_aces = 0
    pair_kings = 0
    pair_queens = 0
    pair_jacks = 0
...
def check_hand(hand):
    """Vérifier la main pour trouver le gain et payer en multipliant."""
    if hand[0] == 'blank':
        return

    if check_royal_flush(hand):
        Pok.plyr_winnings = Pok.plyr_stake *250
        return 10
...
# Crée des boutons de mise, de tirage et de distribution.
...
def about_menu():
    """information sur le programme depuis l'interface."""
...
def quit_prog():
    """Message confirmation fermeture du programme."""
...
FILE_MENU.add_command(label='À propos', command=about_menu)
...
#lancement jeu.
def start_game():
    """Commencement et execution dans l'ordre."""
    load_high_score()
    disply_blanks()
    set_hold_btns()
    Pok.no_card_holding = True
...
# Première pioche et affichage élément de la partie.
hand = [Pok.card_one, Pok.card_two, Pok.card_three, Pok.card_four, Pok.card_five]
hand_value = check_hand(hand)
high_score_lbl = Label(high_score_frame, font=('Helvetica', 10, 'bold'),
                       text='Top Score: €'+str(Pok.high_score))
